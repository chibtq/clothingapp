    ...
    private void loadOrderDetails() {
        ...
        getOkHttpClient().newCall(orderRequest).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() -> Toast.makeText(OrderConfirmationActivity.this, "Failed to load order details", Toast.LENGTH_SHORT).show());
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (orders == null || orders.isEmpty()) {
                        runOnUiThread(() -> Toast.makeText(OrderConfirmationActivity.this, "Order not found", Toast.LENGTH_SHORT).show());
                        return;
                    }
                    ...
                    getOkHttpClient().newCall(userRequest).enqueue(new Callback() {
                        @Override
                        public void onFailure(Call call, IOException e) {
                            runOnUiThread(() -> Toast.makeText(OrderConfirmationActivity.this, "Failed to load user info", Toast.LENGTH_SHORT).show());
                        }
                        ...
                        public void onResponse(Call call, Response userResponse) throws IOException {
                            ...
                            if (userResponse.isSuccessful()) {
                                ...
                                getOkHttpClient().newCall(addressRequest).enqueue(new Callback() {
                                    @Override
                                    public void onFailure(Call call, IOException e) {
                                        runOnUiThread(() -> Toast.makeText(OrderConfirmationActivity.this, "Failed to load address", Toast.LENGTH_SHORT).show());
                                    }
                                    ...
                                    public void onResponse(Call call, Response addressResponse) throws IOException {
                                        ...
                                        if (addressResponse.isSuccessful()) {
                                            ...
                                            getOkHttpClient().newCall(orderItemsRequest).enqueue(new Callback() {
                                                @Override
                                                public void onFailure(Call call, IOException e) {
                                                    runOnUiThread(() -> Toast.makeText(OrderConfirmationActivity.this, "Failed to load order items", Toast.LENGTH_SHORT).show());
                                                }
                                                ...
                                                public void onResponse(Call call, Response orderItemsResponse) throws IOException {
                                                    ...
                                                    if (orderItemsResponse.isSuccessful()) {
                                                        ...
                                                        runOnUiThread(() -> {
                                                            ...
                                                            if (address != null) {
                                                                binding.shippingAddressText.setText("Address: " + address.getFullAddress());
                                                            }

                                                            // Hiển thị danh sách sản phẩm
                                                            binding.orderItemsContainer.removeAllViews();
                                                            for (OrderItem item : orderItems) {
                                                                View itemView = LayoutInflater.from(OrderConfirmationActivity.this).inflate(android.R.layout.simple_list_item_2, binding.orderItemsContainer, false);
                                                                TextView text1 = itemView.findViewById(android.R.id.text1);
                                                                TextView text2 = itemView.findViewById(android.R.id.text2);
                                                                text1.setText(item.getProduct().getName() + " (Size: " + item.getSize() + ")");
                                                                text2.setText("Quantity: " + item.getQuantity() + " | Price: " + String.format("%.2f₫", item.getPrice()));
